...
(* Note: had to reference the solution in order to start working on this module,
   as I had no idea how to start to write a module.
   Also, note nothing within the module ends with ;; *)
module IntListStack =
  struct
...
(* Note the solution opened IntListStack locally, which is better *)
...
(* Note: no idea what was going on here until checked the solution. *)
...
(* Note: the input is of type unit. *)
